...
public class Contiguous implements VfsInterface {

	@Override
	public boolean createFile(Directory dir, String name, int sizeKB,
		...
		for (Space space : spaces) {
			if (sizeKB <= space.size && !space.state) {
				...
				VirtualFile file = new VirtualFile(name, allocatedBlocks);
				dir.files.add(file);
	...
	public int deleteFile(Directory dir, String name, ArrayList<Space> spaces,
		...
		for (VirtualFile file : dir.files) {
	...
	public void readTree(system sys, ObjectInputStream os, int currentSize,
		...
		if (currentSize < sizeKB-1) {
			...
			for (int i = 0; i < fileListSize; i++) {
				...
				VirtualFile file = new VirtualFile(s[s.length - 1], allocatedBlocks);
				files.add(file);
				currentSize += size;
			}
			dir.setFiles(files);
